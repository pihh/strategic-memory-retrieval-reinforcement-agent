...
class StrategicMemoryAgent:
    ...
    def reset_trajectory(self):
        """Clears internal trajectory buffer (used between episodes)."""
        self.trajectory = []

    def run_episode(self, her_target=None):
        """
        Runs a full episode in the environment, collecting experience for PPO.

        Args:
            her_target: (optional) Used for Hindsight Experience Replay (if enabled).

        Returns:
            dict: Trajectory and diagnostic data from the episode.
        """
    ...
    def learn(self, total_timesteps=2000, log_interval=100):
        """
        Runs main PPO training loop, collecting rollouts and optimizing agent.

        Args:
            total_timesteps (int): Train until this many steps are seen.
            log_interval (int): Print log/metrics every N episodes (if verbose).
        """
        steps = 0
        episodes = 0
        all_returns = []
